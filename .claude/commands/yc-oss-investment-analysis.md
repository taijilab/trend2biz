...
v3 变更：新增「媒体与热度」维度；新增「公司/组织背景」调查与报告章节；强化数据校验流程；增加产品分类防误判规则；牵引力评分增加反虚高机制。
...
按顺序执行，每步 1-3 次 web_search / web_fetch，总计 8-18 次：
...
2. **公司/组织背景调查（如果仓库属于组织，必须执行）**
   ...
   - `web_fetch` `https://github.com/{org}` → 组织简介、官网链接、公开成员数、公开仓库数
   - `web_search` "{组织名} company" → 公司官网、注册地、成立时间、业务范围
   - `web_search` "{组织名} funding investors crunchbase" → 融资轮次、金额、投资方、估值
   - `web_search` "{组织名} team size employees" → 团队规模、招聘情况
   - `web_search` "{组织名} product revenue customers" → 产品线、已知客户、收入信号
   - 需要搞清楚的关键问题：
     - 该仓库是公司的**核心产品**还是**副产品/开源贡献**？
     - 公司的商业模式是什么？开源项目在商业版图中的战略角色？
     - 公司有多少个公开仓库？该项目是否是最重要的？
     - 公司是否有其他收入来源（非开源相关）？
...
3. **项目官网**：官网链接 → 产品定位、定价页面、团队页面、用户案例
...
5. **融资信息**：`web_search` "{公司名} funding valuation investors" → Crunchbase / PitchBook / 新闻

6. **竞品搜索（必须基于真实产品类别）**
   - 基于第 2 步确认的产品定位搜索**同类产品**
   - ⚠️ **竞品必须和项目解决同一个问题**
   - `web_search` "{真实产品类别} alternatives competitors funding"

7. **市场数据**：`web_search` "{准确细分市场名} market size TAM 2025 2026"
...
| 组织/公司基本信息（若属于组织） | 搜索结果 | 公司背景部分标注"⚠️ 信息不足" |
...
| 团队规模 | {人数 / 范围 / ⚠️ 未知} |
| GitHub 公开仓库数 | {数量} |
| 该项目在公司中的角色 | {核心产品 / 副产品 / 开源贡献 / 内部工具开源化} |
...
### 商业版图
{公司有哪些产品线？开源项目在其中扮演什么角色？是获客工具、核心技术、还是社区建设？公司是否有其他收入来源？}

### 公司 vs 项目风险提示
{公司层面是否有额外风险（如资金链、战略转向、关键人物变动）？项目命运是否过度依赖公司？}
